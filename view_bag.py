...
def df_to_np(df, b=1):
    ...
    for i in range(1, len(df) - 1):
        if df.iloc[i].type == "control":
            # TODO add check for ensuring stacking errors do not occur
            if df.iloc[i - 1].type == "state" and df.iloc[i + 1].type == "state":
                x_pre = df.iloc[i - 1].val
                c = df.iloc[i].val
...
def plot_np_data(X, b=1):
    ...
    states_pre = np.array([item[:state_length] for item in X])
    states_pos = np.array([item[:-state_length] for item in X])
    controls = np.array(
        [item[state_length : state_length + control_length] for item in X]
    )

    # Plotting states
    ax = fig.add_subplot(1, 2, 1, projection="3d")
    colors = mpl.cm.viridis(np.linspace(0, 1, len(states_pre)))
    ax.scatter(states_pre[:, 0], states_pre[:, 1], states_pre[:, 2], color=colors)
    # ax.scatter(states_pos[:, 0], states_pos[:, 1], states_pos[:, 2], color=colors)
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
